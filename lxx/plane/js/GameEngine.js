...
var gameEngine = {
    ...
    start: function () {
        ...
        gameEngine.loading(function () {
            ...
            {
                gameEngine.myplane = new MyPlane(gameEngine.interval);
                gameEngine.myplane1 = new MyPlane(gameEngine.interval);
                gameEngine.myplane.autofire();
                gameEngine.myplane1.autofire();
            }
            //自动开火
            // gameEngine.myplane.autofire();
            // gameEngine.myplane1.autofire();
        ...
        document.onkeydown = function (evt) {
            ...
            if(gameEngine.ren==2){
                ...
                self.myplane.move(8);
                    ...
                    self.myplane1.move(-8);//调用飞机移动的方法
                }else if(code == 68){//->
                    self.myplane1.move(8);
                }
            }
            if(gameEngine.ren==1){
                if(code == 37){//<-
        ...
        var timer = setInterval(function () {
            ...
            for(var i in self.enemyList){
                ...
                if(gameEngine.ren==2){
                    ...
                    if(isCrash(self.myplane1.ele,self.enemyList[i].ele))
                    {//所有敌机和MyPlane进行碰撞检测
                       // self.myplane1.boom();
                        flag2=true;               
                    }

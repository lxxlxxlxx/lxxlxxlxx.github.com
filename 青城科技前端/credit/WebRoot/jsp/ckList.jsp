					...
					<div class="col-sm-12">
						<section class="panel">
			                <div class="panel-body">
			                  <a href="<%=basePath%>ckAction_beforeAddOrUpdateCk?ckid=" class="btn btn-info"><i class="fa fa-plus"></i>&nbsp;&nbsp;添加催款信息</a>
			                </div>
		              </section>
					</div>
				</div>
                  ...
                  <div class="panel-body">
                     <div class="adv-table">
                        <table  class="display table table-striped table-advance table-hover table-bordered" id="hidden-table-info">
                           ...
                           <tbody>
                              <c:forEach items="${ckList }" var="obj">
                              	 <tr>
                              	 	 <td>${obj.id }</td>
	                                 <td>${obj.uname }</td>
	                                 <td>${obj.rq }</td>
	                                 ...
	                                 <td>${obj.sdate }</td>
                                	<td>
		                             	<a class="btn btn-primary btn-xs" href="<%=basePath%>ckAction_beforeAddOrUpdateCk?ckid=${obj.id}"><i class="fa fa-pencil"></i></a>
		                                <a class="btn btn-danger btn-xs" data-toggle="modal" data-id="1" href="#myModal2" onclick="deleteRow('${obj.id }');"><i class="fa fa-times"></i></a>
		                                <a class="btn btn-danger btn-xs" onclick="downloadfile('${obj.id }');">下载</a>
		                             </td>
	                              </tr>
                              </c:forEach>
                           </tbody>
...
<div class="modal fade" id="myModal2" >
	<div class="modal-dialog">
		<div class="modal-content">
			<form action="<%=basePath%>ckAction_deleteCk" method="post">
				<input type="hidden" name="ckDelId" id="delid" value="">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
	                 <h4 class="modal-title">确认删除该催款信息？</h4>
	            </div>
...
<script >
    ...
    function downloadfile(id){
    	$.ajax({ 
    		url: "<%=basePath%>ckAction_downloadfile",
    		dataType:"text",
    		type:"POST",
    		data:{ckid:id},
    		success: function(){
    			window.open("http://localhost:8080/credit/file/123.doc");
			}
		});
    }
</script>
